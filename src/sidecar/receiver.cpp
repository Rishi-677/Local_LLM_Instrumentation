...
struct SocketGuard {
    ...
    ~SocketGuard() { if (fd != kInvalidSocket) CLOSE_SOCKET(fd); }
...
bool SidecarReceiver::parse_float_array(const std::string& s, size_t& i,
...
bool SidecarReceiver::extract_attention(const std::string& line,
                                         const TensorEvent& ev,
    ...
    if (ap.rows <= 0 || ap.cols <= 0) return false;
    ...
    size_t idx = p;
    if (!parse_float_array(line, idx, ap.weights)) return false;

    // Validate the flattened size matches rows * cols.
    if ((int)ap.weights.size() != ap.rows * ap.cols) return false;
...
bool SidecarReceiver::parse_line(const std::string& line, TensorEvent& out) {
    ...
    while (i < line.size()) {
        ...
        if (key == "node_name") {
            std::string v; parse_string(line, i, v); copy_into(e.node_name, v);
        } else if (key == "op_name") {
            std::string v; parse_string(line, i, v); copy_into(e.op_name, v);
        } else if (key == "shape") {
            if (i < line.size() && line[i] == '[') ++i;
            for (int d = 0; d < kMaxDims; ++d) {
                std::string t = read_token(line, i);
                if (!t.empty()) e.shape[d] = static_cast<int64_t>(std::strtoll(t.c_str(), nullptr, 10));
                skip_ws(line, i);
                if (i < line.size() && line[i] == ',') ++i;
            }
            if (i < line.size() && line[i] == ']') ++i;
            ...
            std::string t = read_token(line, i);
            if (key == "id")            e.id = static_cast<uint64_t>(std::strtoull(t.c_str(), nullptr, 10));
            else if (key == "timestamp_ns") e.timestamp_ns = static_cast<uint64_t>(std::strtoull(t.c_str(), nullptr, 10));
            else if (key == "layer_idx")   e.layer_idx = static_cast<int32_t>(std::strtol(t.c_str(), nullptr, 10));
            else if (key == "op_class")    e.op_class = static_cast<OpClass>(std::strtol(t.c_str(), nullptr, 10));
            else if (key == "device")      e.device = static_cast<Device>(std::strtol(t.c_str(), nullptr, 10));
            else if (key == "dtype")       e.dtype = static_cast<int32_t>(std::strtol(t.c_str(), nullptr, 10));
            else if (key == "dtype_size")  e.dtype_size = static_cast<int32_t>(std::strtol(t.c_str(), nullptr, 10));
            else if (key == "latency_us")  e.latency_us = static_cast<uint64_t>(std::strtoull(t.c_str(), nullptr, 10));
            else if (key == "v_min")       e.v_min = std::strtof(t.c_str(), nullptr);
            else if (key == "v_max")       e.v_max = std::strtof(t.c_str(), nullptr);
            else if (key == "v_mean")      e.v_mean = std::strtof(t.c_str(), nullptr);
            else if (key == "v_std")       e.v_std = std::strtof(t.c_str(), nullptr);
            else if (key == "l2_norm")     e.l2_norm = std::strtof(t.c_str(), nullptr);
            else if (key == "sparsity")    e.sparsity = std::strtof(t.c_str(), nullptr);
            else if (key == "has_nan")     e.has_nan = (t == "true" || t == "1");
            else if (key == "has_inf")     e.has_inf = (t == "true" || t == "1");
            else if (key == "stats_valid") e.stats_valid = (t == "true" || t == "1");
            else if (key == "payload_id")  e.payload_id = static_cast<uint32_t>(std::strtoul(t.c_str(), nullptr, 10));
        }
...
SidecarReceiver::SidecarReceiver(EventRing& ring, Config cfg)
    : ring_(ring), cfg_(cfg) {}

SidecarReceiver::~SidecarReceiver() {
    int fd = fd_.exchange(-1, std::memory_order_relaxed);
    if (fd != -1) CLOSE_SOCKET(static_cast<socket_t>(fd));
}
...
void SidecarReceiver::run(std::atomic<bool>& alive) {
    ...
    if (::bind(listen_fd, reinterpret_cast<struct sockaddr*>(&addr), sizeof(addr)) == kSocketError) {
    ...
    while (alive.load(std::memory_order_acquire)) {
        if (!read_line(conn_fd, line, alive)) break;
