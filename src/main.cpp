...
void apply_cli(int argc, char** argv, Options& o) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--config")
            o.config_path = next("--config");
        else if (a == "--print-config")
            o.print_config = true;
        else if (a == "--record")
            o.record = next("--record");
        else if (a == "--replay")
            o.replay = next("--replay");
        else if (a == "--max-tokens")
            o.max_tokens = std::atoi(next("--max-tokens").c_str());
        else if (a == "--delay-ms")
            o.delay_ms = std::atoi(next("--delay-ms").c_str());
        else if (a == "--ring-capacity")
            o.ring_capacity = (size_t)std::strtoull(next("--ring-capacity").c_str(), nullptr, 10);
        else if (a == "--anomaly-threshold")
            o.anomaly_thresh = (float)std::atof(next("--anomaly-threshold").c_str());
        else if (a == "--flash-attn")
            o.no_flash_attn = false;
        else if (a == "--no-flash-attn")
            o.no_flash_attn = true;
        else if (a == "--headless")
            o.headless = true;
        else if (a == "--preview")
            o.preview = true;
        else if (a == "--preview-filter") {
            o.preview = true;
            o.preview_filter = next("--preview-filter");
        } else if (a == "--bench")
            o.bench = true;
        else if (a == "--export-csv") {
            o.headless = true;
            o.export_csv = next("--export-csv");
        } else if (a == "--export-json") {
            o.headless = true;
            o.export_json = next("--export-json");
        } else if (a == "-h" || a == "--help")
            o.help = true;
        else
            pos.push_back(a);
...
static void consumer_loop(ts::EventRing& ring, ts::UiState& ui, ts::App* app, ts::Topology& topo,
                          ts::AnomalyDetector& detector, ts::Recorder* recorder,
                          ts::AttentionSink& attn_sink, std::atomic<int>& target_layer,
                          std::atomic<int>& pending_select, std::atomic<bool>& alive) {
...
static void inference_loop(const Options& opt, ts::EventRing& ring, ts::UiState& ui,
                           ts::AttentionSink* attn_sink, std::atomic<bool>& alive,
                           std::atomic<bool>& done) {
    ...
    cparams.flash_attn_type =
        opt.no_flash_attn ? LLAMA_FLASH_ATTN_TYPE_DISABLED : LLAMA_FLASH_ATTN_TYPE_AUTO;
    ...
    int n = llama_tokenize(vocab, opt.prompt.c_str(), (int)opt.prompt.size(), tokens.data(),
                           (int)tokens.size(), true, false);
    if (n < 0) {
        tokens.resize(-n);
        n = llama_tokenize(vocab, opt.prompt.c_str(), (int)opt.prompt.size(), tokens.data(),
                           (int)tokens.size(), true, false);
    }
...
static void replay_loop(const Options& opt, ts::EventRing& ring, ts::UiState& ui,
                        std::atomic<bool>& alive, std::atomic<bool>& done) {
    DoneGuard guard{done};
    ts::Replay r(opt.replay);
    if (!r.ok()) {
        ui.set_session_state("REPLAY OPEN FAILED");
        return;
    }
...
static int run_bench(const Options& opt) {
    ...
    std::printf(
        "  note: most overhead is structural — cb_eval runs the graph\n"
...
int main(int argc, char** argv) {
    ...
    std::string cfg_err;
    ts::Config cfg = opt.config_path.empty() ? ts::Config::defaults()
    ...
    if (opt.print_config) {
        std::fputs(cfg.to_string().c_str(), stdout);
        return 0;
    }
    ...
    spdlog::info("Local_LLM_Instrumentation starting (theme={}, ring_capacity={})", cfg.theme,
                 opt.ring_capacity);
    ...
    auto start_producer = [&]() {
        ...
        return std::thread(inference_loop, std::cref(opt), std::ref(ring), std::ref(ui), &attn_sink,
                           std::ref(alive), std::ref(producer_done));
    };
    ...
    if (opt.headless) {
        ...
        if (got_attention)
            std::printf("  attention captured  : yes (%dx%d)\n", att_rows, att_cols);
        else
            std::printf("  attention captured  : no\n");
    ...
    if (opt.preview) {
        ...
        ts::App app(ui);
        if (!opt.preview_filter.empty())
            app.set_stream_filter(opt.preview_filter);
    ...
    std::thread consumer(consumer_loop, std::ref(ring), std::ref(ui), &app, std::ref(topo),
                         std::ref(detector), recorder.get(), std::ref(attn_sink),
