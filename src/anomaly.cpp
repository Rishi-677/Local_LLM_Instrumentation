...
AnomalyRecord AnomalyDetector::push(uint64_t ts, int severity, std::string text) {
    ...
    if (ledger_.size() > kLedgerCap) {
        ledger_.erase(ledger_.begin(), ledger_.begin() + (ledger_.size() - kLedgerCap));
    }
    return rec;
}
...
std::optional<AnomalyRecord> AnomalyDetector::inspect(const ts::TensorEvent& e) {
    ...
    if (e.has_nan) {
        return push(e.timestamp_ns, 2,
                    "NaN in " + node + " (layer " + std::to_string(e.layer_idx) + ")");
    }
    ...
    if (e.stats_valid &&
        (std::fabs(e.v_max) > cfg_.extreme_abs || std::fabs(e.v_min) > cfg_.extreme_abs)) {
        if (outlier_seen_.insert(node).second) {
            return push(e.timestamp_ns, 1,
                        "Outlier Feature " + node + ": Max " + std::to_string(e.v_max));
        }
        return std::nullopt;
    }

    // Rule 4: unexpected CPU fallback (opt-in; deduped once per node).
    if (cfg_.flag_cpu_fallback && e.device == ts::Device::CPU && e.layer_idx >= 0 &&
        e.op_class != ts::OpClass::Embed) {
        if (cpu_seen_.insert(node).second) {
            return push(e.timestamp_ns, 1,
                        "CPU fallback: " + field_str(e.op_name) + " on host memory");
        }
    }
    return std::nullopt;
}
