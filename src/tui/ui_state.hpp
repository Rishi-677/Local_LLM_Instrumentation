...
// Caps — keep memory bounded; old data is the least useful in a live debugger.
constexpr size_t kStreamCap   = 500;
constexpr size_t kAnomalyCap  = 200;

class UiState {
public:
    // ---- Plain snapshot the render reads (a copy, no lock needed) ----------
    struct Snapshot {
    ...
    // ---- Producer side (lead's consumer thread) ---------------------------
    ...
    void push_event(const TensorEvent& ev) {
        ...
        while (stream_.size() > kStreamCap) stream_.pop_front();
        ++total_events_;
    }
    ...
    void set_topology(std::vector<LayerNode> topo) {
        ...
        if (selected_ >= static_cast<int>(topology_.size())) {
            selected_ = topology_.empty() ? 0
                                          : static_cast<int>(topology_.size()) - 1;
        }
    }
    ...
    void push_anomaly(const Anomaly& a) {
        ...
        while (anomalies_.size() > kAnomalyCap) anomalies_.erase(anomalies_.begin());
    }
    ...
    // ---- Consumer side (render thread + input handler) --------------------
