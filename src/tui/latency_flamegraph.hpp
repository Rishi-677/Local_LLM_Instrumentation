...
inline bool is_latency_flame_bucket(const LayerNode& n) {
    return n.label.rfind("layers.", 0) == 0 || n.label == "embed" || n.label == "output" ||
           n.label == "other";
}

inline std::vector<LatencyFlameRow> build_latency_flamegraph(const std::vector<LayerNode>& topology,
                                                             std::size_t limit = 10) {
    std::vector<LatencyFlameRow> rows;
    rows.reserve(topology.size());

    for (const LayerNode& n : topology) {
        if (!is_latency_flame_bucket(n) || n.total_latency_us == 0)
            continue;
        rows.push_back(LatencyFlameRow{n.layer_idx, n.label, n.total_latency_us});
    }

    std::stable_sort(rows.begin(), rows.end(),
                     [](const LatencyFlameRow& a, const LatencyFlameRow& b) {
...
inline std::string latency_ascii_bar(uint64_t value, uint64_t max_value, std::size_t width = 24) {
    if (width == 0)
        return {};
    if (value == 0 || max_value == 0)
        return std::string(width, '.');
    const long double ratio = static_cast<long double>(value) / static_cast<long double>(max_value);
