...
std::pair<OpClass, int32_t> classify(const char * tensor_name) {
    ...
    if (has(buf, "embd") || has(buf, "embed") || has(buf, "inp_tokens")) {
        ...
        cls = OpClass::MLP;
    } else if (has(buf, "attn") || has(buf, "kqv") || has(buf, "kq") ||
               has(buf, "wqkv") ||
               buf[0] == 'q' || buf[0] == 'k' || buf[0] == 'v') {
        cls = OpClass::Attn;
    } else if (has(buf, "norm")) {
        cls = OpClass::Norm;
    } else if (has(buf, "result_output") || has(buf, "logits") ||
               has(buf, "result")) {
        cls = OpClass::Output;
    }
...
LayerNode & Topology::layer_node(int32_t layer_idx, const std::string & label) {
    for (auto & c : root_.children) {
        if (c.layer_idx == layer_idx && c.label == label) return c;
    }
    root_.children.push_back(
        LayerNode{ layer_idx, label, OpClass::Other, false, 0, 0, {} });
    return root_.children.back();
}

LayerNode & Topology::op_node(LayerNode & parent, OpClass cls) {
    for (auto & c : parent.children) {
        if (c.op_class == cls) return c;
    }
    parent.children.push_back(
        LayerNode{ parent.layer_idx, to_string(cls), cls, false, 0, 0, {} });
...
int32_t Topology::selected() const { return selected_; }

const LayerNode & Topology::root() const { return root_; }
